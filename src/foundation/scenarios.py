...
from src.foundation.base.environment import scenario_registry
...
class LayoutFromFile(BaseEnvironment):
    ...
    layout file (see ./maps/ for examples) to determine the spatial arrangement of
    ...
    Args:
        ...
        env_layout_file (str): Name of the layout file in ./maps/ to use.
    ...
    def __init__(
        ...
        **base_env_kwargs,
        ...
        # Load in the layout
        path_to_layout_file = Path(f"{Path(__file__).parent}/maps/{env_layout_file}")
        ...
        if self.fixed_four_skill_and_loc:
            ...
            n_in_group = np.zeros(4, dtype=int)
    ...
    def get_current_optimization_metrics(self):
        ...
        for agent in self.world.agents:
            curr_optimization_metric[agent.idx] = utilities.isoelastic_coin_minus_labor(
        ...
        if self.planner_reward_type == "coin_eq_times_productivity":
            curr_optimization_metric[self.world.planner.idx] = (
                utilities.coin_eq_times_productivity(
            ...
            curr_optimization_metric[self.world.planner.idx] = (
                utilities.inv_income_weighted_coin_endowments(
                    coin_endowments=np.array(
                        [agent.total_endowment("Coin") for agent in self.world.agents]
                    )
                )
            ...
            curr_optimization_metric[self.world.planner.idx] = (
                utilities.inv_income_weighted_utility(
                    coin_endowments=np.array(
                        [agent.total_endowment("Coin") for agent in self.world.agents]
                    ),
                    utilities=np.array(
    ...
    def scenario_metrics(self):
        ...
        metrics["social/productivity"] = utilities.get_productivity(coin_endowments)
        metrics["social/equality"] = utilities.get_equality(coin_endowments)

        utilities = np.array(
            [self.curr_optimization_metric[agent.idx] for agent in self.world.agents]
        )
        metrics["social_welfare/coin_eq_times_productivity"] = (
            utilities.coin_eq_times_productivity(
                coin_endowments=coin_endowments, equality_weight=1.0
            )
        )
        metrics["social_welfare/inv_income_weighted_coin_endow"] = (
            utilities.inv_income_weighted_coin_endowments(
                coin_endowments=coin_endowments
            )
        )
        metrics["social_welfare/inv_income_weighted_utility"] = (
            utilities.inv_income_weighted_utility(
        ...
        for agent in self.all_agents:
            for resource, quantity in agent.inventory.items():
                metrics["endow/{}/{}".format(agent.idx, resource)] = (
